...
pub fn find_invalid_chars(chars: &Vec<char>, i: usize, open_stack: &mut Vec<char>, invalid: &mut Vec<char>) -> bool {
    ...
    /*if open.is_none() {
        ...
        let is_open = match curr {
            '(' | '[' | '{' | '<' => true,
            _ => false,
        };

        if is_open {
            return find_invalid_chars(chars, i + 1, Some(curr), invalid);
        } else {
            invalid.push(curr);
            return false;
        }
    ...
    /*let new_open = match (is_closed, curr) {
        (true, _) => None,
        (_, '(' | '[' | '{' | '<') => Some(curr),
        _ => open,
    };*/
